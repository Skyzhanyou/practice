...
$(function () {
    ...
    $('#np-s').on('click', function () {
        ...
        setInterval(function () {
            $('#test').text('进度：' + $.AMUI.progress.status)
        }, 1000);
    });
    ...
    $('.am-slider').flexslider({
        ...
        //String: "horizontal" or "vertical" 选择slide滚动形式，"horizontal": 为水平滚动，"vertical": 上下滚动。
        reverse: false,
        //{NEW} Boolean: true or false 翻转slide子项运动方向，"false": 默认为正常运动方向, "true": 为相反方向。
        animationLoop: true,
        //Boolean: true or false Should the animation loop? If false, directionNav will received "disable" classes at either end
        smoothHeight: false,
        //{NEW} Boolean: true or false 当slide图片比例不一样时，"true": 父类自动适应图片高度，"false": 不自动适应，父类高度为图片的最高高度，默认为false。
        startAt: 0, //Integer: 开始显示的slide顺序，0为第一张slide。
        slideshow: true, //Boolean: true or false 是否自动播放，默认为true。
        slideshowSpeed: 2000, //Integer: 单位为 ms 自动播放的间隔时间。
        animationSpeed: 600, //Integer: 单位为 ms 动画运动时间，配合参数slideshowSpeed可以做出一直滚动效果。
        initDelay: 0, //{NEW} Integer: 单位为 ms 首次执行动画的延迟时间，默认为0
        randomize: false,
        //Boolean: true or false 是否随机slide顺序，默认为false。

        // Usability features
        pauseOnAction: true,
        //Boolean: true or false 作用于控制点暂停自动播放程序。
